...
Scratch space for small tools built around Claude Code itself — reading its on-disk state, scripting over sessions, etc.

## Quick start: `cct` (interactive TUI)
...
```bash
bash claude-dev/install.sh      # symlinks cct into ~/.local/bin
cct                              # run from any directory

bash claude-dev/uninstall.sh           # remove symlink
bash claude-dev/uninstall.sh --purge   # also delete ~/.cache/cct
bash claude-dev/install.sh --force     # replace a foreign cct in ~/.local/bin
```
